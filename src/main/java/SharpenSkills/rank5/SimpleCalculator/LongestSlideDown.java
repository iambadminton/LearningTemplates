 ...
 * argument and returns its' longest 'slide down'. For example,
 * longestSlideDown [[3], [7, 4], [2, 4, 6], [8, 5, 9, 3]]
 * // => 23
 * ###By the way... My tests include some extraordinarily high pyramides so as you can guess, brute-force method is a bad
    ...
    public static int longestSlideDown(int[][] p) {
        for (int i = p.length - 1; i >= 1; i--)
            for (int j = 0; j < i; j++) {
                ...
                System.out.println("=====>p[i-1][j] =" + p[i - 1][j] + " i = " + i + " j=" + j + "  Math.max(p[i][j], p[i][j + 1])=" + Math.max(p[i][j], p[i][j + 1]));
